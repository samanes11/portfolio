...
function MediaModal({
  ...
  const fields: {
    ...
    { key: "image", label: "Image URL", placeholder: "./bicy.png" },
    {
      key: "category",
      label: "Category",
      placeholder: "Mobile / Web / MicroService",
    },
  ];
  ...
  return (
    <div className="fixed inset-0 z-[1100] bg-black/40 backdrop-blur-sm">
      <WindowFloat
        onclose={onClose}
        title={isEdit ? "Edit Media" : "Add New Media"}
        maxWidth="380px"
        padding={20}
      ...
      >
        <div className="flex flex-col gap-4" style={{ direction: "ltr" }}>
          ...
          <div className="flex flex-col gap-2">
            <div
              className={`self-start px-3 py-1 rounded-full bg-gradient-to-r ${
                categoryGradient[item.category] ?? "from-gray-400 to-gray-500"
...
export default function AdminMediaManagement({
  ...
  async function handleSave(form: Partial<MediaItem>) {
    ...
    const res = await fetch(
      `/api/system/${isEdit ? "updateMedia" : "addMedia"}`,
      {
        method: "POST",
  ...
  return (
    <>
                ...
                {media.map((item) => (
                  ...
                  >
                    <div className="relative aspect-video bg-gray-900 overflow-hidden">
                      ...
                      />
                      <div
                        className={`absolute top-2 left-2 px-2.5 py-1 rounded-full bg-gradient-to-r ${categoryGradient[item.category] ?? "from-gray-400 to-gray-500"} text-xs text-white font-semibold shadow-xl`}
                      >
                        {item.category}
                      </div>
      ...
      {modal === "preview" && selected && (
        <PreviewModal item={selected} onClose={() => setModal(null)} />
      )}
